...
describe('auth-api', () => {
  ...
  it('refreshes an expired access token once and retries a bearer call', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse(200, { csrf_token: 'csrf-1' }))
      .mockResolvedValueOnce(jsonResponse(200, { access_token: 'at-old', token_type: 'Bearer', expires_in: 300 }))
      .mockResolvedValueOnce(jsonResponse(200, { id: 'u1', email: 'a@b.test', first_name: 'Camille', email_verified: false }))
      .mockResolvedValueOnce(jsonResponse(401, { error: 'invalid_token' }))
      .mockResolvedValueOnce(jsonResponse(200, { access_token: 'at-new', token_type: 'Bearer', expires_in: 300 }))
    ...
    const retryCalls = fetchMock.mock.calls.filter((c) => String(c[0]).endsWith('/v1/auth/verify/request'));
  ...
  it('surfaces the 401 without retrying when the refresh yields no token', async () => {
    ...
    const api = await loadApi();
    await api.login('a@b.test', 'une longue phrase');
    await expect(api.requestVerificationEmail()).rejects.toMatchObject({ status: 401, code: 'invalid_token' });
    const endpointCalls = fetchMock.mock.calls.filter((c) => String(c[0]).endsWith('/v1/auth/verify/request'));
  ...
  it('does not refresh or retry a non-bearer mutation on 401', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock);
    const api = await loadApi();
    await expect(api.confirmEmailVerification('tok')).rejects.toMatchObject({ status: 401, code: 'invalid_token' });
