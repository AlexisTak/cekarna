...
- [x] Créer un tableau de supervision local séparé du site candidat. L’application Tauri contrôle les adresses de bouclage depuis Rust et peut démarrer ou arrêter les services Cekarna explicitement autorisés : les services Compose `auth` et `notifications`, sans supprimer leurs volumes, et les processus API, offres et Ollama qu’elle a elle-même lancés. La préparation locale des offres synchronise un jeton ignoré par Git et charge uniquement les fixtures synthétiques.
