...
import { fetchCandidateWorkspace, saveCandidateWorkspace } from './auth-api';
...
it('stores an authenticated dossier under its account without overwriting the guest workspace', async () => {
  ...
  vi.mocked(fetchCandidateWorkspace).mockResolvedValue({
    workspace: personal,
    revision: 0,
  });
  ...
  try {
    ...
    expect(
      JSON.parse(localStorage.getItem(`${STORAGE_KEY}.account.user-b`)!),
    ).toEqual(personal);
...
it('transfers a guest dossier only after the person confirms it', async () => {
  const guest = emptyWorkspace();
  guest.profile.firstName = 'Invité';
  localStorage.setItem(STORAGE_KEY, JSON.stringify(guest));
  vi.mocked(fetchCandidateWorkspace).mockResolvedValue(null);
  vi.mocked(saveCandidateWorkspace).mockResolvedValue(1);
  vi.spyOn(window, 'confirm').mockReturnValue(true);
  const container = document.createElement('div');
  const root = createRoot(container);
  try {
    await act(async () => root.render(<App />));
    expect(saveCandidateWorkspace).toHaveBeenCalledWith(guest, 0);
    expect(container.textContent).toContain('Invité');
  } finally {
    await act(async () => root.unmount());
  }
});
